...
const UpdateEmail = ({ username }) => {
  ...
  const submitInfo = e => {
    ...
    if (!new_email.trim() || !email_conf.trim()) {
      ...
      if (debug) {
        console.log(
          "I should never be seen but user tried to submit with an empty field"
        );
      }
      return;
    }
    ...
    if (!emailError && !emailConfError) {
      ...
      axios
        .put(apiURL, payload)
        .then(response => {
          // If email doesn't exists, push changes
      ...
      if (debug) {
        console.log("Something went really wrong if you can see me");
      }
    }
  };
