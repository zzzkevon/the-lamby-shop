  ...
  */
  ...
  const validateNewPassword = () => {
    ...
    if (new_password.trim() === "") {
      ...
      setNewPasswordError(
        "Password must contain at least 1 uppercase letter and 1 number, and be 8-32 characters long"
      );
      return false;
    } else {
      setNewPasswordError("");
  ...
  const submitInfo = async e => {
    ...
    if (!newPasswordError && !passwordConfError) {
      ...
      */
      const apiURL =
        "https://xgj9xa22l3.execute-api.us-west-2.amazonaws.com/Prod/cognito";
      const payload = {
        ...
        axios
          ...
          .then(response => {
            ...
            if (response.status === 200) {
              ...
              setServerDenial(
                "Password was not able to be changed. Please try again after checking that your current password is correct."
              );
            }
          })
          .catch(error => {
  ...
  return (
    <div className="main-bg just-another-hand 4xl">
      <div className="flex flex-col justify-start items-center">
        ...
        {/** submit button */}
        <button
          className="bg-[#780000] hover:bg-[#780000] text-white py-2 px-4 rounded-full mt-4"
          onClick={submitInfo}
        ...
        {currentPasswordError && (
          <span className="text-red-500 text-3xl">{currentPasswordError}</span>
        )}
        {newPasswordError && (
          <span className="text-red-500 text-3xl" data-testid="new-pass-err">
            {newPasswordError}
          </span>
        )}
        {passwordConfError && (
          <span className="text-red-500 text-3xl" data-testid="conf-pass-err">
            {passwordConfError}
          </span>
        )}
        {serverDenial && (
          <span className="text-red-500 text-6xl">{serverDenial}</span>
        )}
      </div>
    </div>
  );
